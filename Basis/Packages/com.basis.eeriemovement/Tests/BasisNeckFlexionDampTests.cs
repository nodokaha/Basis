...
namespace Basis.Tests.IK
{
    ...
    public class BasisNeckFlexionDampTests
    {
        ...
        static Quaternion Axis(float deg, float3 axis)
            => (Quaternion)quaternion.AxisAngle(axis, math.radians(deg));
        ...
        static Vector3 Neck(Quaternion headRot, float extensionDamp, float flexionDamp)
            => BasisNeckCueCore.Solve(Vector3.zero, headRot, Lever, Vector3.up, extensionDamp, flexionDamp);
        ...
        public void FlexionDampZeroIsTheOldBehaviour_ByteForByte()
        {
            for (float deg = -90f; deg <= 90f; deg += 7.5f)
            {
                ...
                Assert.That((old - now).magnitude, Is.EqualTo(0f),
                    $"the five-argument overload must be exactly the six-argument one with flexion 0, at {deg} deg");
            }
        }
        ...
        public void FlexionDampNeverTouchesTheLookUpSide()
        {
            ...
            for (float deg = 0f; deg <= 85f; deg += 5f)
            {
                ...
                Assert.That((Neck(up, 0.65f, 0f) - Neck(up, 0.65f, 1f)).magnitude, Is.EqualTo(0f),
                    $"look-up at {deg} deg must be untouched by the flexion damp");
            }
        }
        ...
        public void ExtensionDampNeverTouchesTheLookDownSide()
        {
            for (float deg = 0.5f; deg <= 85f; deg += 5f)
            {
                ...
                Assert.That((Neck(down, 0f, 0.5f) - Neck(down, 1f, 0.5f)).magnitude, Is.EqualTo(0f),
                    $"look-down at {deg} deg must be untouched by the extension damp");
            }
        }
        ...
        public void PureYawIsUntouchedByEitherDamp()
        {
            for (float deg = 0f; deg < 360f; deg += 30f)
            {
                ...
                Assert.That((Neck(rot, 0.65f, 0.5f) - undamped).magnitude, Is.LessThan(1e-6f),
                    $"a head turn moves no part of the nod lever, at {deg} deg");
            }
        }
        ...
        public void DampingCutsTheLookDownNeckRise_AndTheDefaultVirtuallyRemovesIt()
        {
            ...
            foreach (float deg in new[] { 30f, 45f, 60f, 75f })
            {
                ...
                float damped = Neck(Pitch(deg), 0.65f, 0.5f).y - restY;
                float full = Neck(Pitch(deg), 0.65f, 1f).y - restY;

                Assert.That(undamped, Is.GreaterThan(0.01f),
                    $"the artefact has to exist at {deg} deg or this file stops measuring anything");
                Assert.That(Mathf.Abs(damped), Is.LessThan(Mathf.Abs(undamped) * 0.6f),
                    $"the default damp must at least halve the rise at {deg} deg");
                Assert.That(Mathf.Abs(full), Is.LessThan(1e-6f),
                    $"full damping means the neck bone does not move at all on a nod, at {deg} deg");
            }
        }
        ...
        public void TheLeverKeepsItsLength_ItIsALinkInTheSpineChain()
        {
            ...
            for (float deg = -85f; deg <= 85f; deg += 5f)
            {
                ...
                Assert.That(damped.magnitude, Is.EqualTo(Lever.magnitude).Within(1e-5f),
                    $"lever length must survive damping at {deg} deg");
            }
        }
        ...
        public void TheNeckIsContinuousThroughLevelGaze()
        {
            ...
            for (float deg = -2f; deg <= 2f; deg += 0.1f)
            {
                ...
                Assert.That((current - previous).magnitude, Is.LessThan(2e-3f),
                    $"discontinuity crossing level gaze at {deg} deg");
                previous = current;
            }
        }
        ...
        public void TheVerticalPoleIsHandledOnBothSides()
        {
            ...
            Vector3 restLever = Lever;

            Vector3 straightDown = Neck(Pitch(90f), 0.65f, 1f);
            Assert.That((straightDown - restLever).magnitude, Is.LessThan(1e-3f),
                "fully damped, straight down must leave the lever at rest");

            Vector3 straightUp = Neck(Pitch(-90f), 1f, 0.5f);
            Assert.That((straightUp - restLever).magnitude, Is.LessThan(1e-3f),
                "fully damped, straight up must leave the lever at rest");
        }
        ...
        public void TheSwingIsStillRemovedWhenTheViewpointSitsAtOrBelowTheHeadBone()
        {
            ...
            foreach (float aboveHead in new[] { 0.05f, 0.005f, 0f, -0.02f, -0.05f })
            {
                ...
                foreach (float pitch in new[] { -75f, -60f, -30f, 30f, 60f })
                {
                    ...
                    float expected = arm.y * Mathf.Sin(p) + arm.z * Mathf.Cos(p) - arm.z;
                    Assert.That(Swing(arm, pitch).ForwardMeters, Is.EqualTo(expected).Within(1e-5f),
                        $"arm y={aboveHead} at {pitch} deg");
                }
            }
        }
